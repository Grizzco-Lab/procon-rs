...
- `src/cuttlefish.rs` + `web/cuttlefish.js`: Cuttlefish, the VOD reviewer. Videos: a session segment, a local file or a YouTube range (yt-dlp into `[cuttlefish] cache`); comments at times/ranges with drawings; reviews as JSON in `[cuttlefish] reviews`; `POST /api/cuttlefish/ai` answers 501 until the `cuttlefish` crate is wired in (key from `ANTHROPIC_API_KEY` only). `web/sketch.js` is the drawing layer both use (shapes in 0–1 coordinates).
