...
- `procon` (`src/bin/main.rs`, config `config.toml`): the studio on the Linux host with the capture card. Web dashboard with four apps: Studio (live preview, controller view incl. 3D, recording with sound and game settings, replay, data, motion), Inkspector (recorded sessions frame by frame: labels, delays, sound, overlays, predictions, object labeling), Cuttlefish (VOD reviews with comments, drawings and the AI; Knowledge tab over the `cuttlefish` store) and Vision (detection and tracking with `gameplay-vision`, send to labels). Every project is used from the page, not the CLI.
...
- `src/web.rs`: warp server with the page embedded from `web/`, WebSocket `/ws` (`state` per input report, `status` twice a second, preview fMP4 as binary), `POST /api/command`, `/api/inspect/...`, `/api/cuttlefish/...`, `/api/vision/...`.
...
- `src/cuttlefish.rs` + `web/cuttlefish.js`: Cuttlefish, the VOD reviewer. Videos: a session segment, a local file or a YouTube range; comments at times/ranges with drawings. Each review is a folder `[cuttlefish] reviews/<id>/` with `review.json` and, when the video lives there, `video.<ext>` (`video.file`, a plain file name checked to stay in the folder): a YouTube range is downloaded by yt-dlp straight into a new review (title, channel, upload date saved in `video`), a local file can be copied in (`POST reviews/<id>/copy`), a session segment is never copied. Deleting removes the folder. No download cache; `Cuttlefish::migrate` moves flat `<id>.json` reviews into folders at startup, with their videos from the old `~/.cache/procon-cuttlefish/yt-<hash>.mp4` (`cache_id`). `POST /api/cuttlefish/ai` sends the range's frames (ffmpeg, 2 fps, 720p) and nearby comments to `cuttlefish::review::review` with the shared store of `src/knowledge.rs` (from `[cuttlefish] knowledge`) and returns its comments; 501 while it cannot start (key from `ANTHROPIC_API_KEY` only). `web/sketch.js` is the drawing layer both use (shapes in 0–1 coordinates).
- `src/knowledge.rs` + `web/knowledge.js`: Cuttlefish's Knowledge tab (`#cuttlefish/view=knowledge`, tabs in the top bar). The store and E5 embedder load once (`Knowledge::loaded`) and serve search, imports and the reviewer (`cuttlefish::review::review/ask` over borrowed parts); the model client is made per request from `ANTHROPIC_API_KEY` (501 without; the page only learns set/not set). Imports (`cuttlefish::ingest`, a `Sink` that logs into the job) run one at a time on a thread, with cancel. The crate cannot delete documents, so neither can the page.
- `src/vision.rs` + `web/vision.js`: Vision (`#vision/s=&seg=&n=`). One run at a time on a thread (`POST /api/vision/run`, poll `GET job`, `POST cancel`): `gameplay_vision` frames → `Detector` (loaded once per model/device and reused) → `Tracker` (ids copied onto detections by IoU), per-frame decode/network/total ms; results in `[vision] results` as `<session>/<stem>.objects.jsonl` (label format + `"ms"`) and `.run.json`. `POST send` maps classes (`labels::map_classes`) and merges into the annotations by the prelabel rule under the labeling mode's lock (`Annotations::merge_model_boxes`); the link opens `#inspect/...&label=1`. `cuda` feature forwards to `gameplay-vision/cuda`.

**Dashboard (`web/`)**: one page, four apps switched by hash (`#studio`, `#inspect/...`, `#cuttlefish/...`, `#vision/...`) without reloading; app links as a left rail or a top-bar switch (`data-nav`); the View popover sets `data-theme` (studio, joy, telemetry), `data-layout` (phone) and `data-nav`, stored in `localStorage` (`procon-*`). The Studio's preview pauses and its views stop drawing while another app is shown or the tab is hidden; the other apps stop polling and playback likewise (background jobs keep running on the server). `web/controller3d.js`: three.js from jsdelivr, extrudes the SVG view's outline and reuses its theme colors; the SVG is the fallback without WebGL or the CDN. The input overlay (`drawInputHud` in `app.js`) draws sticks, buttons and turn rates over the Studio preview (held back by the preview delay) and the Inkspector frame.
